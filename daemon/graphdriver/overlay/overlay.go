...
func (d *Driver) Create(id string, parent string) (retErr error) {
	...
	/*	if err := os.MkdirAll(path.Dir(dir), 0755); err != nil {
			return err
		}
	*/
...
// MountPath return container mount path
func (d *Driver) MountPath(id string) string {
	return path.Join(d.home, id, "merged")
}

// Register for external running container when daemon restart
