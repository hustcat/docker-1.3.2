...
const (
	...
	DockerCloneSnapshotPrefix = "clone-snapshot"
)

func getSnapshotName(id string) string {
	return DockerCloneSnapshotPrefix + id
}
...
func createDockerSnapshot(ioctx RadosIoCtx, id, snapshotName string) error {
	...
	if err := rbdSnapshotCreate(img, snapshotName); err != nil {
		return err
	}
...
func cloneImage(ioctx RadosIoCtx, id, parent string) error {
	snaphostName := getSnapshotName(parent)
	img, err := rbdOpenSnapshot(ioctx, parent, snaphostName)
	if err != nil {
		if err != RbdNotFoundError {
			return err
		}

		if err := createDockerSnapshot(ioctx, parent, snaphostName); err != nil {
			return err
		}
	...
	if err := rbdSnapshotProtect(img, snaphostName); err != nil {
		if err != RbdBusyError {
			return err
		}
	}

	if err := rbdClone(ioctx, parent, snaphostName, id); err != nil {
		return err
	}
...
func createBaseImageIfNeeded(ioctx RadosIoCtx, pool, id string) error {
	snaphostName := getSnapshotName(id)
	if img, err := rbdOpenSnapshot(ioctx, id, snaphostName); err == nil {
		defer rbdClose(img)
...
func deleteDockerSnapshotIfPresent(ioctx RadosIoCtx, id string) error {
	snaphostName := getSnapshotName(id)
	img, err := rbdOpenSnapshot(ioctx, id, snaphostName)
	if err != nil {
		if err == RbdNotFoundError {
			return nil
		}
	...
	if err := rbdSnapshotUnprotect(img, snaphostName); err != nil {
	...
	if err := rbdSnapshotRemove(img, snaphostName); err != nil {
		if err != RbdNotFoundError {
			return err
		}
	}
